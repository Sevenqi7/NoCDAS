// Copyright (c) 2026
//
// Description: RTL-owned functional Attention leaf for cNoC type5 flits.
...
module mfu_alu_attention #(
  ...
  logic [FLIT_W-1:0] attention_flit_comb;
  flit_meta_t attention_meta_comb;
  ...
  assign active_tokens =
      (ctx_i.kv_token_count == 16'd0) ? 16'd0 :
      (ctx_i.kv_token_count < max_tokens_from_sram) ? ctx_i.kv_token_count :
  ...
  assign is_attention_op = op_i.is_attention && (op_i.opcode == OP_ATTENTION);
  assign attention_context_ready =
      query_valid_q[ctx_i.out_sel][ctx_i.vc_id] || head_like;
  ...
  always_comb begin : proc_attention_operands
    ...
    for (operand_task_idx = 0;
      ...
      task_id_by_slot[operand_task_idx] =
          ctx_i.task_ids_flat[operand_task_idx * TASK_ID_W +: TASK_ID_W];
    end
    ...
    for (operand_dim_idx = 0; operand_dim_idx < MAX_K_DIM; operand_dim_idx = operand_dim_idx + 1) begin
      query_value_by_dim[operand_dim_idx] =
          query_q[ctx_i.out_sel][ctx_i.vc_id][operand_dim_idx];
  ...
  always_ff @(posedge clk) begin
    if (reset) begin
      ...
      if (data_wr_i.valid && data_wr_i.bank_sel) begin
        for (wr_lane_idx = 0; wr_lane_idx < 32; wr_lane_idx = wr_lane_idx + 1) begin
          if (data_wr_i.byte_en[wr_lane_idx] &&
              (data_wr_i.addr + wr_lane_idx) < SRAM_DEPTH) begin
            kv_bank[data_wr_i.addr + wr_lane_idx] <=
                data_wr_i.data[wr_lane_idx * 8 +: 8];
          end
        end
      end

      if (start_i && is_attention_op) begin
        // Head/head-tail starts a fresh query stream for this output/VC.
        if (head_like) begin
          query_valid_q[ctx_i.out_sel][ctx_i.vc_id] <= 1'b1;
          attention_state_valid_q[ctx_i.out_sel][ctx_i.vc_id] <= 1'b1;
          attention_running_max_q[ctx_i.out_sel][ctx_i.vc_id] <= -32'sd2147483647;
          attention_running_sum_q[ctx_i.out_sel][ctx_i.vc_id] <= 32'sd0;
          for (q_lane_idx = 0; q_lane_idx < MAX_K_DIM; q_lane_idx = q_lane_idx + 1) begin
            query_q[ctx_i.out_sel][ctx_i.vc_id][q_lane_idx] <= 8'd0;
          end
        end
        ...
        for (q_lane_idx = 0; q_lane_idx < 32; q_lane_idx = q_lane_idx + 1) begin
          ...
          if ((q_lane_idx < effective_payload_len) &&
            ...
            query_q[ctx_i.out_sel][ctx_i.vc_id][global_query_idx] <=
        ...
        if (tail_like && attention_context_ready) begin
          if (tail_score_valid) begin
            attention_running_max_q[ctx_i.out_sel][ctx_i.vc_id] <= new_running_max_q4;
            attention_running_sum_q[ctx_i.out_sel][ctx_i.vc_id] <= new_running_sum_q4;
          end else begin
            attention_running_max_q[ctx_i.out_sel][ctx_i.vc_id] <= -32'sd2147483647;
            attention_running_sum_q[ctx_i.out_sel][ctx_i.vc_id] <= 32'sd0;
          end
      ...
      if (release_state_i && is_attention_op && tail_like) begin
        ...
        attention_state_valid_q[ctx_i.out_sel][ctx_i.vc_id] <= 1'b0;
        attention_running_max_q[ctx_i.out_sel][ctx_i.vc_id] <= -32'sd2147483647;
        attention_running_sum_q[ctx_i.out_sel][ctx_i.vc_id] <= 32'sd0;
      end
    end
  ...
  always_comb begin : proc_attention_writeback
    ...
    attention_flit_comb = pkt_flit_i;
    attention_meta_comb = pkt_meta_i;
    task_id = 16'd0;
    target_idx = 17'd0;
    target_lane = 6'd0;
    old_output_q = 32'sd0;
    ...
    if (is_attention_op && tail_like && attention_context_ready && active_tokens != 16'd0) begin
      ...
      for (task_slot_idx = 0; task_slot_idx < MAX_TASKS; task_slot_idx = task_slot_idx + 1) begin
        ...
        if ((task_slot_idx < ctx_i.task_count) &&
          ...
          attention_flit_comb[target_lane * 8 +: 8] = wb_sat_byte_tmp;
        end
      end

      if (new_running_max_q4 > 32'sd127) begin
        attention_meta_comb.header_reserved[7:0] = 8'h7f;
      end else if (new_running_max_q4 < -32'sd128) begin
        attention_meta_comb.header_reserved[7:0] = 8'h80;
      end else begin
        attention_meta_comb.header_reserved[7:0] = new_running_max_q4[7:0];
      end

      if (new_running_sum_q4 > 32'sd127) begin
        attention_meta_comb.header_reserved[15:8] = 8'h7f;
      end else if (new_running_sum_q4 < -32'sd128) begin
        attention_meta_comb.header_reserved[15:8] = 8'h80;
      end else begin
        attention_meta_comb.header_reserved[15:8] = new_running_sum_q4[7:0];
      end
    end
  end
...
`ifndef SYNTHESIS
  always_ff @(posedge clk) begin
    if (!reset) begin
      if (start_i && is_attention_op && !head_like &&
          !attention_state_valid_q[ctx_i.out_sel][ctx_i.vc_id]) begin
      ...
      if (release_state_i && is_attention_op && tail_like &&
        ...
        if (!query_valid_q[ctx_i.out_sel][ctx_i.vc_id]) begin
...
`endif
  ...
  always_ff @(posedge clk) begin : proc_attention_registers
    if (reset) begin
      valid_o <= 1'b0;
      attention_flit_o <= '0;
      attention_meta_o <= '0;
    end else begin
      valid_o <= start_i && is_attention_op;
      if (start_i && is_attention_op) begin
        attention_flit_o <= attention_flit_comb;
        attention_meta_o <= attention_meta_comb;
      end else if (release_state_i && is_attention_op && tail_like) begin
