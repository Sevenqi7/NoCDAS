...
module mfu_alu_add #(
  ...
  always_comb begin : proc_add_result
    ...
    for (pair_idx = 0; pair_idx < 16; pair_idx = pair_idx + 1) begin
      if ((((pair_idx << 1) + 1) < effective_payload_len) &&
          pair_update_mask[pair_idx[3:0]]) begin
        lhs_q = {{24{flit_i[((pair_idx << 4) + 7)]}}, flit_i[(pair_idx << 4) +: 8]};
        rhs_q = {{24{flit_i[((pair_idx << 4) + 15)]}},
                 flit_i[((pair_idx << 4) + 8) +: 8]};
        ...
        flit_tmp[(pair_idx << 4) +: 8] = sat_byte_tmp;
      end
    end

    scalar_byte_tmp = flit_tmp[data_lane_byte_offset +: 8];
